...
async def animation_command(client, message):  # Note the change to `async def`
    try:
        ...
        await message.reply_animation(animation=gif_url, caption=msg)  # Await is now valid as the function is async

    except Exception as e:
        await message.reply_text(f"An unexpected error occurred: {str(e)}")  # Add await here as well
